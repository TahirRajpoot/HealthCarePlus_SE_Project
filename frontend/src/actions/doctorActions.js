...
export const doctorLoginAction =
  (email, password) => async (dispatch, getState) => {
    try {
      dispatch({ type: DOCTOR_LOGIN_REQUEST });
      const { data } = await axios.post(
        "http://localhost:8080/api/doctors/loginHospital",
        {
          email,
          password,
        }
...
export const doctorRegisterAction =
  (name, email, speciality, contact, organization, graduatedFrom, password) =>
  async (dispatch) => {
    try {
      dispatch({
      ...
      const { data } = await axios.post(
        "http://localhost:8080/api/doctor/doc_register",
        {
          name,
          email,
          speciality,
