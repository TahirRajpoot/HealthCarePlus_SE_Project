...
const HospitalDescriptionPublic = ({ hospitalData }) => {
  ...
  useEffect(() => {
    if (hospitalData) {
      const ratingArray =
        hospitalData?.reviews?.map((review) => review.ratings) || [];
      ...
      <CardHeader>
        <NameAndRatingContainer>
          <ProfilePictureContainer>
            {hospitalData && (
              <ProfilePicture
                src={returnURLFromObjectOfStrings(hospitalData.hospitalImages)}
        ...
        <ApplyButtonContainer>
          <AppointmentButton outlined onClick={() => handleModalOpen()}>
