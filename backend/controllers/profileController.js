...
const Patient = require("../models/patientModels.js");
const Doctor = require("../models/doctorModels.js");
...
const isDoctorValid = (newdoctor) => {
  ...
  if (!newdoctor.confirmPassword) {
    errorList[errorList.length] =
      "Please re-enter password in Confirm Password field";
  }
  ...
  if (!doctorValidStatus.status) {
    ...
    try {
      const updateddoctor = await Doctor.updateOne(
        { _id: req.params.id },
        { $set: { phone: req.body.phone, department: req.body.department } }
      );
